...
class Image_model extends CI_Model
{
  ...
  public function idProyek()
    ...
    public function idUser()
      ...
      $cekdb = $this->db->get_where('user')->row_array();
      $header = $cekdb['id_user'];
      $u=date('ymd');
      if (!$cekdb) {
        $header = 1;
        $notrans='U0'.$u.'00'.$header;
      }else  {
        $notrans= substr($header,2,6);
        if ($u != $notrans) {
          $header = 1;
      ...
      public function idJalan($id)
        {
        $this->db->order_by('id_jalan', 'DESC');
        $cekdb = $this->db->get_where('jalan', ['id_desa' => $id])->row_array();
        $header = $cekdb['id_jalan'];
        $u=date('ymd');
        if (!$cekdb) {
          $header = 1;
          $notrans='j0'.$u.$id.'00'.$header;
        }else  {
          $notrans= substr($header,2,6);
          if ($u != $notrans) {
            $header = 1;
