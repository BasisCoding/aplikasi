...
public function registrasi()
{
  if ($this->session->userdata('email')) {
    ...
    $this->form_validation->set_rules('nama','Nama', 'required|trim',[
      ...
      $this->form_validation->set_rules('id_kecamatan','id_kecamatan', 'required|trim',[
          'required' => 'Tidak boleh kosong'
        ]);
  ...
  if ($this->form_validation->run() == false) {
    $data['id_user'] = $this->Image_->idUser();
...
private function _sendEmail($token, $type)
{
  ...
  $config = [
    ...
    'smtp_user' => 'pupr.kab@gmail.com',
    'smtp_pass' => 'puprkab2020',
    'smtp_port' => 465,
    'mailtype' => 'html',
    'charset' => 'utf-8'
  ];
  ...
  //pengiriman email
  $this->email->from('pupr.kab@gmail.com', 'Dinas Pupr Serang');
  ...
  if ($user) {
    ...
    if ($user_token) {
      if (time() - $user_token['date_created'] < (60*60)) {
        ...
        $user = $this->db->get_where('user',['email' => $email])->row_array();
        $this->db->delete('desa', ['id_user' => $user['id_user']]);
        $this->db->delete('user', ['email' => $email]);
        $this->db->delete('user_token', ['email' => $email]);
        $this->session->set_flashdata('pesan', '<div class="alert alert-danger" role="alert">
              Token kadaluarsa Silahkan Registrasi Ulang
            </div>');
            redirect('auth/login');
      }
    }else {
